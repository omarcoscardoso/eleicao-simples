...
function  show_enquete($id_ENQUETE,$sessao){
  ...
  $html_enquete.='<br>';
  $html_enquete.='<input type="number" id="opcao" name="opcao" maxlength="2" value="" autocomplete="off" autofocus="autofocus" style="font-size:90px; width: 120px !important;">';
  $html_enquete.='<br>';
  $html_enquete.='<br>';
...
function show_cont($id_ENQUETE,$sessao){
  $sql = 'SELECT votos 
            FROM votou 
           WHERE votou.id_enquete="'.$id_ENQUETE.'" 
             and votou.sessao="'.$sessao.'" 
           order by votos desc limit 1';

  $sqlVoto=mysql_query($sql) or die(mysql_error());
